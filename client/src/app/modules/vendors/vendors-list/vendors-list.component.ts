...
import { MockServerVendorsService } from './paging/mock-server-vendors-service';
import { vendorModal } from './paging/model/vendor.modal';
...
export class VendorsListComponent implements OnInit {
  ...
  constructor(private serverResultsService: MockServerVendorsService) {
    this.pageNumber = 0;
  }
  ...
  setPage(pageInfo: PageInfo) {
    ...
    this.serverResultsService.getResults(page).subscribe(pagedData => {
      ...
      if (!this.rows) {
        this.rows = new Array<vendorModal>(this.totalElements || 0);
      }
